...
const seedMarketplaceItems = [
  ...
  },
  {
    ...
    name: "Tickets",
    slug: "tickets",
    description: "Trouble ticket management for support requests, issue tracking, and customer service.",
    longDescription: "The Tickets app adds a full-featured trouble ticket system to your ORIGIN workspace. Track support requests through customizable status workflows, assign agents, set priorities, and manage SLAs. Includes a customer portal for public ticket submission.",
    icon: "ticket",
    isFree: true,
    price: 0,
    version: "0.1.0",
    status: "published",
    category: "apps",
    tags: ["app", "tickets", "support", "helpdesk", "customer-service"],
    author: "ORIGIN",
    docSlug: "help-app-tickets",
  },
];
...
- Adjust theme colors through the Theme Editor`,
  ...
  {
    title: "ORIGIN App Add-on Standard & Generator",
    slug: "dev-origin-app-addon-standard",
    content: `The App Add-on Standard defines how all ORIGIN apps are structured and shipped.

## App Contract

Every app uses \`defineOriginApp()\` from \`shared/originApps/types.ts\` to declare its key, entitlement, nav items, API paths, and marketplace config.

## Registry

\`shared/originApps/registry.ts\` is the single source of truth. It exports an array of all apps and helper getters.

## Generator

Run \`npx tsx scripts/gen-app.ts --key <key> --name "<Name>"\` to scaffold a new app add-on with server module, UI page, and documentation.

## Gating

All routes use \`requireEntitlement()\`. UI pages handle 403 gracefully. Super Admins bypass checks.

## See Also

- \`docs/ORIGIN_APP_ADDON_STANDARD.md\` for the full specification`,
    category: "development",
    type: "dev",
    tags: ["apps", "generator", "standard", "architecture"],
    sortOrder: 200,
    isPublished: true,
  },
  {
    title: "Managing Apps in ORIGIN",
    slug: "help-managing-apps",
    content: `Apps extend your ORIGIN workspace with additional features like CRM, Tickets, and more.

## What Are Apps?

Apps are optional add-ons that add new capabilities to your workspace. They appear in the Apps section of your sidebar once enabled.

## How to Enable an App

1. Go to the **Marketplace** in your sidebar
2. Browse or search for the app you want
3. Click **Install** (free apps) or **Purchase** (paid apps)
4. The app will appear in your sidebar under **Apps**

## How to Disable an App

1. Go to the **Marketplace**
2. Click on the installed app
3. Click **Uninstall** to disable it

Disabling an app does not delete your data — it simply hides the app from your workspace.

## Available Apps

- **CRM** — Manage leads, contacts, and customer relationships
- **Tickets** — Track support tickets and customer service requests

## Safety

- Disabling an app never breaks your live website
- Your data is preserved even when an app is disabled
- Apps can be re-enabled at any time`,
    category: "general",
    type: "help",
    tags: ["apps", "marketplace", "getting-started"],
    sortOrder: 10,
    isPublished: true,
  },
  {
    title: "Tickets App",
    slug: "help-app-tickets",
    content: `The Tickets app adds trouble ticket management to your ORIGIN workspace.

## What's Included

- Ticket inbox for managing support requests
- Status workflow (Open, In Progress, Resolved, Closed)
- Agent assignment for team workload management
- Customer portal for public ticket submission

## Getting Started

Install the Tickets app from the Marketplace, then find it under **Apps > Tickets** in your sidebar.
...
- Reporting and analytics`,
    category: "marketplace",
    type: "help",
    tags: ["app", "tickets", "support"],
    sortOrder: 107,
    isPublished: true,
  },
  {
    title: "Tickets App — Developer Guide",
    slug: "dev-app-tickets",
    content: `Developer documentation for the Tickets app add-on.

## Architecture

- Server: \`server/modules/apps/tickets/\`
- Client: \`client/src/pages/app-tickets.tsx\`
...
| Method | Path | Description |
|--------|------|-------------|
| GET | \`/api/apps/tickets/health\` | Health check |

## Gating

Uses \`requireEntitlement("apps.tickets")\` middleware. Super Admins bypass.`,
    category: "development",
    type: "dev",
    tags: ["app", "tickets", "development"],
    sortOrder: 201,
    isPublished: true,
  },
  {
